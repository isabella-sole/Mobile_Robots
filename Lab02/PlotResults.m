...
% Graphs displayed:
% Figure 1: 
%   - The path calculated by odometry only (red). Odometry uses constant
%       radii equal to the initial value, so the result may differ from
%       what ShowOdometry plots (ShowOdometry uses nominal radii).
...
% Figure 6:
%   - Top: right and left radii estimations as functions of time.
%   - Bottom: right and left radii standard deviations as functions of
%   time.
% Figure 7: 
...
% Usage: PlotResults 
% Remark: Files 'inputLog.mat' and 'log.txt' must be present, which
% requires prior execution of MagnetLoc.m 
...
% of the file, here calcPhase, t, x, y, theta, rr, rl, P11...P55, U1, U2, Y1, Y2.
...
% Odometry only estimated path using the INITIAL value of the radii.
Xodo(:,1) = [x(1) ; y(1) ; theta(1)] ;
deltaD        = zeros(nbPeriods,1) ;
deltaTheta    = zeros(nbPeriods,1) ;
for i = 1 : nbPeriods
    deltaD(i)        = (1/2)*( rr(1)*U(1,i) + rl(1)*U(2,i) )             ;
    deltaTheta(i)    = rr(1)/trackGauge*U(1,i) - rl(1)/trackGauge*U(2,i) ;
    Xodo(:,i+1) = Xodo(:,i) + ...
        [ deltaD(i)*cos(Xodo(3,i)) ;
          deltaD(i)*sin(Xodo(3,i)) ;
          deltaTheta(i) ] ;
end
travDistance = zeros(1,nbPeriods) ;
for i = 2 : nbPeriods
    travDistance(i) = travDistance(i-1) + deltaD(i) ;
end
...
title({'Estimated path EKF (blue) and odometry (red)','(Odometry with initial radii)'});
xlabel('x (mm)');
ylabel('y (mm)');
...
figure; 
subplot(2,1,1);
plot( tOdo,deltaD/samplingPeriod , 'LineWidth',2 );
xlabel('t (s)')
ylabel('v (mm/s)');
title({'Odometry-estimated speed (top)', 'and rotation speed (bottom)','with initial radii'});
zoom on ; grid on;
subplot(2,1,2);
plot( tOdo,deltaTheta*180/pi/samplingPeriod , 'LineWidth',2 );
xlabel('t (s)')
...
figure; 
plot( tMagnetDetection , dMahaAll(1,:) , 'b.' , 'LineWidth',2 ) ;
for k = 2:5
    hold on; 
    plot( tMagnetDetection , dMahaAll(k,:) , 'r.' , 'LineWidth',2 ) ;
end
hold on;
...
hold on ;
plot( t,sigrr , 'LineWidth',2 );
xlabel('t (s)')
ylabel('sigma_{rR} (mm)');
title('Right radius std dev.');
zoom on ; grid on;
...
hold on ;
plot( t,sigrl , 'LineWidth',2 );
xlabel('t (s)')
ylabel('sigma_{rL} (mm)');
title('Left radius std dev.');
zoom on ; grid on;
...
fprintf('\nTotal travelled distance: %d mm\n',round(sum(abs(deltaD))));
fprintf('Final odometry error: %3.1f %%\n\n', ...
    (norm([x(size(x,1));y(size(y,1))]-Xodo(1:2,size(Xodo,2))) / sum(abs(deltaD)) )*100 );
